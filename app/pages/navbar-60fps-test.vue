<template>
  <div class="min-h-screen bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-black">
    ...
    <div class="fixed bottom-4 left-4 z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">性能监控</h3>
        <div class="space-y-1 text-xs">
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-gray-400">FPS:</span>
            <span class="font-mono" :class="fpsColor">{{ currentFPS }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-gray-400">状态切换:</span>
            <span class="font-mono text-blue-500">{{ isScrolled ? '已滚动' : '顶部' }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-gray-400">版本:</span>
            <span class="font-mono text-purple-500">{{ currentVersion }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 当前导航栏 -->
    <component :is="currentNavbarComponent" @scroll-state-change="handleScrollStateChange" />
    ...
    <div class="container mx-auto px-4 py-16">
      <section class="text-center py-32">
        ...
        <p class="text-xl text-gray-600 dark:text-gray-300 mb-8">
          快速滚动测试导航栏性能，体验丝滑切换效果
        </p>
        <div
          class="h-96 bg-gradient-to-r from-purple-400 to-pink-400 rounded-3xl flex items-center justify-center"
        >
      ...
      <section class="space-y-16">
        <div v-for="i in 20" :key="i" class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg">
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">测试章节 {{ i }}</h2>
          <p class="text-gray-600 dark:text-gray-300 mb-6">
            这是第 {{ i }} 个测试内容块，用于验证导航栏在滚动时的性能表现。 当前使用：<strong
              class="text-blue-500"
              >{{ currentVersion === 'original' ? '原版 CSS 动画' : '60 FPS 优化版' }}</strong
            >
          </p>
          <div
...
const fpsColor = computed(() => {
  if (currentFPS.value >= 55) return 'text-green-500'
  if (currentFPS.value >= 30) return 'text-yellow-500'
  return 'text-red-500'
})
